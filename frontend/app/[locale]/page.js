...
const CONFIG = {
  ...
  /**
   * IMPORTANT:
   * poster MUST be an IMAGE (jpg/png/webp). A .webm poster will not work properly.
   */
  video: {
    mp4: process.env.NEXT_PUBLIC_HERO_VIDEO || "/video/hero.mp4",
  ...
  /**
   * ✅ Hero dynamic media (mix of videos + images)
   * Put ONLY public paths that exist in /public
   */
  hero: {
  imageIntervalMs: 6500,
  videoFallbackMs: 12000,
  pauseOnHover: true,
  media: [
    // ✅ VIDEOS (correct paths)
    { src: "/video/hero.mp4", alt: "Hero video mp4" },
    { src: "/brand/urus.JPG", alt: "Uros" },
    { src: "/brand/qalasaya.JPG", alt: "Qalasaya" },
    { src: "/brand/pukara.JPG", alt: "Pukara" },
    ],
},
...
const DynamicComponents = {
  HeroSearch: NextDynamic(() => import("@/app/components/landing/HeroSearch"), {
    ssr: false,
    loading: () => <LoadingSpinner />,
  }),
  InteractivePeruMap: NextDynamic(() => import("@/app/components/landing/InteractivePeruMap"), {
    ssr: false,
    loading: () => <LoadingSpinner />,
  }),
  HeroLanding: NextDynamic(() => import("@/app/components/landing/HeroLanding"), {
    ssr: false,
    loading: () => <LoadingSpinner />,
  }),
  TourGuide3D: NextDynamic(() => import("@/app/components/landing/TourGuide3D"), {
    ssr: false,
    loading: () => <LoadingSpinner />,
  }),
  promotioncarousellocale: NextDynamic(() => import("@/app/components/PromotionCarouselLocale"), {
    ssr: false,
    loading: () => <LoadingSpinner />,
  }),

  // ✅ Must exist: frontend/app/components/landing/HeroMediaRotator.jsx
  HeroMediaRotator: NextDynamic(() => import("@/app/components/landing/HeroMediaRotator"), {
    ssr: false,
  }),
};
  ...
  return (
    <div className="group bg-white rounded-2xl p-8 shadow-sm border border-slate-100 hover:shadow-xl hover:border-[#0086C0]/30 transition-all duration-500 hover:-translate-y-2">
      <div className="flex items-start gap-5">
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#0086C0] to-[#0E374A] flex items-center justify-center flex-shrink-0 shadow-sm group-hover:scale-110 transition-transform duration-300">
          <span className="text-white text-xl font-black" style={{ fontFamily: "'Bree Serif', serif" }}>
            {index}
          </span>
        </div>
        <div className="flex-1">
          <h3 className="font-black text-[#0E374A] mb-3 text-l" style={{ fontFamily: "'Bree Serif', serif" }}>
            {title}
          </h3>
          <p className="text-[#64748B] leading-relaxed font-medium" style={{ fontFamily: "'Bree Serif', serif" }}>
            {description}
          </p>
        </div>
      </div>
...
function StatCard({ value, label }) {
  return (
    <div className="group text-center p-8 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl hover:border-[#0086C0]/30 transition-all duration-500 hover:-translate-y-2">
      ...
      <div className="text-[#64748B] text-base font-bold" style={{ fontFamily: "'Bree Serif', serif" }}>
        {label}
      </div>
    </div>
  );
...
/**
 * ✅ Updated HeroSection:
 * - Uses HeroMediaRotator to rotate images/videos
 * - Autoplay video works because muted + playsInline
 * - Rotates images by interval; videos on ended + fallback
 */
function HeroSection({ lang, t }) {
  return (
    <section className="relative h-screen w-full overflow-hidden">
      <DynamicComponents.HeroMediaRotator
        media={CONFIG.hero.media}
        poster={CONFIG.video.poster}
        imageIntervalMs={CONFIG.hero.imageIntervalMs}
        videoFallbackMs={CONFIG.hero.videoFallbackMs}
        pauseOnHover={CONFIG.hero.pauseOnHover}
        className=""
      />

      {/* Optional overlay (keep text readable) */}
      ...
      <div className="relative h-full flex items-center justify-center px-6">
        <div className="max-w-6xl mx-auto text-center">
          ...
          >
            ...
            <span style={{ color: "#A3B117" }}>{t("headline2", "Vive tu próxima aventura")}</span>
          </p>
          ...
          <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 animate-bounce-slow">
            <div className="w-8 h-12 border-2 border-[#A3B117] rounded-full flex items-start justify-center p-2 bg-white/20 backdrop-blur-sm shadow-lg">
              <div className="w-2 h-3 bg-[#A3B117] rounded-full animate-scroll" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-16">
            <SectionTitle>
              ...
              <span className="text-[#A3B117]">{t("destitle", "Tiene Una Riqueza Escondida")}</span>
            </SectionTitle>
            <SectionDescription>
              {t(
                "desdesc",
                "Explora las tres regiones únicas del Perú: Sierra, Costa y Selva. Cada una con su propia magia, historia y aventura."
              )}
            </SectionDescription>
          </div>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-20">
            <SectionTitle>
              ...
              <span className="block text-[#A3B117]">{t("maptitle", "Forma Interactiva")}</span>
            </SectionTitle>
            <SectionDescription>{t("mapdesc", "Haz clic en las regiones para explorar destinos y experiencias únicas")}</SectionDescription>
          </div>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-20">
            <SectionTitle>
              ...
              <span className="block text-[#A3B117]">{t("specialPromotions", "Promociones Especiales")}</span>
            </SectionTitle>
            <SectionDescription>{t("promoDescription", "Tours con descuentos exclusivos disponibles por tiempo limitado")}</SectionDescription>
          </div>

          {promotionalPackages.length > 0 && (
            <DynamicComponents.promotioncarousellocale promotionalPackages={promotionalPackages} lang={lang} />
          )}
