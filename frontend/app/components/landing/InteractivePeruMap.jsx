...
import { 
  MapPin, DollarSign, Clock, Filter, 
  ChevronDown, X, ZoomIn, ZoomOut,
  Navigation, MessageCircle, Globe,
  Tag, Award
} from 'lucide-react';
  ...
  const fetchPackages = useCallback(async () => {
    ...
    try {
      ...
      const processedPackages = listRaw.map((p) => {
        const hasValidCoords = typeof p?.location?.lat === 'number' && typeof p?.location?.lng === 'number';
        let coords;
        
        if (hasValidCoords) {
          coords = { lat: p.location.lat, lng: p.location.lng };
        } else {
          const cityKey = Object.keys(CITY_CENTER).find(
            key => key.toLowerCase() === p?.city?.toLowerCase()
          ) || 'Others';
          coords = CITY_CENTER[cityKey];
        }
        
        const firstImage = Array.isArray(p.media) && p.media.length > 0 
        ...
        return {
          ...p,
          id: p.id || p._id || p.slug || Math.random().toString(36),
          coordinates: coords,
          media: Array.isArray(p.media) ? p.media.map((m) => ({ ...m, url: mediaUrl(m.url) })) : [],
          firstImage,
          durationHours: durationHours || null,
          priceValue: p.effectivePrice || p.price || 0,
        };
      });
...
const PackagePopup = React.memo(({ pkg, onClose, t = (key, fallback) => fallback || key, locale = 'es' }) => {
  ...
  const handleWhatsApp = useCallback(() => {
    const message = t('whatsappMessage', `Hola, estoy interesado en el paquete: ${pkg.title}`).replace('{title}', pkg.title);
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank');
  }, [pkg.title, t]);

  const handleDetails = useCallback(() => {
    window.open(`/${locale}/package/${pkg.slug || pkg.id}`, '_blank');
  }, [pkg.slug, pkg.id, locale]);

  const imageUrl = pkg.media?.[0]?.url || 'https://images.unsplash.com/photo-1587595431973-160d0d94add1?w=400&h=300&fit=crop&q=80&auto=format';
  ...
  return (
    <div className="absolute top-6 left-6 bg-white rounded-2xl shadow-2xl p-6 max-w-sm z-[1000] border-2 border-[#0086C0] animate-fadeInUp">
      ...
      <div className="space-y-5">
        ...
        <div>
          <h3 className="text-xl font-bold mb-3 text-[#0E374A] font-bree">
            {pkg.title}
          </h3>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3 rounded-xl font-bold text-white transition-all hover:shadow-lg hover:scale-105 flex items-center justify-center gap-2 bg-gradient-135 from-[#0086C0] to-[#0E374A] font-bree"
            >
  ...
  return (
    <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
      {stats.map((stat, index) => (
        ...
        >
          <div 
            className="text-4xl font-black mb-2 font-bree"
            style={{ color: stat.color }}
          >
            {stat.value}
          </div>
          <div className="text-sm font-bold text-[#0E374A] font-bree">
            {stat.label}
          </div>
...
const MapLegend = React.memo(({ t = (key, fallback) => fallback || key }) => (
  <div className="absolute bottom-6 left-6 bg-white/95 backdrop-blur-sm rounded-2xl p-5 shadow-xl z-[999] border border-slate-200">
    ...
    <div className="space-y-3">
      ...
      <div className="text-xs text-[#64748B] pl-1 font-bree">
        {t('clickMarkers', 'Haz clic en los marcadores para ver detalles')}
      </div>
    </div>
  </div>
));
...
export default function InteractiveMap() {
  ...
  useEffect(() => {
    if (!leafletMapRef.current || !window.L || filteredPackages.length === 0) return;
    ...
    filteredPackages.forEach(pkg => {
      if (!pkg.coordinates || !pkg.coordinates.lat || !pkg.coordinates.lng) return;

      const lat = parseFloat(pkg.coordinates.lat);
      const lng = parseFloat(pkg.coordinates.lng);
      
      if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
        console.warn('Coordenadas inválidas:', pkg.coordinates);
        return;
      }
      ...
      marker.on('click', () => {
        ...
        leafletMapRef.current.flyTo([lat, lng], 10, {
          duration: 1.2,
          easeLinearity: 0.25
        });
      });
      ...
      marker.on('mouseover', function() {
        ...
        if (icon) {
          const container = icon.querySelector('.marker-container');
          if (container) {
            container.style.transform = 'translateY(-4px) scale(1.05)';
            container.style.filter = 'drop-shadow(0 8px 16px rgba(0, 134, 192, 0.3))';
    ...
    if (validMarkersCount > 0 && bounds.isValid()) {
      try {
        ...
        console.warn('Error en fitBounds:', error);
  ...
  const handlePackageClick = useCallback((pkg) => {
    ...
    setSelectedPackage(pkg);
    if (leafletMapRef.current) {
      leafletMapRef.current.flyTo([pkg.coordinates.lat, pkg.coordinates.lng], 10, {
        duration: 1.2
      });
    }
  ...
  return (
    <div className="w-full bg-gradient-to-b from-white via-[#F8FAFC] to-white px-4 sm:px-6 lg:px-12">
            ...
            <div className="mt-2 rounded-xl bg-white shadow-lg max-h-96 overflow-y-auto border border-slate-200">
              {loading ? (
                ...
                filteredPackages.map((pkg) => (
                  <PackageCard key={pkg.id} pkg={pkg} onClick={handlePackageClick} t={t} />
                ))
              ) : (
        ...
        <div className="p-6 border-b border-slate-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-bold text-lg text-[#0E374A] font-bree">
                {t('packagesAvailable', 'Paquetes Disponibles')}
              </h3>
            </div>
      ...
      <div className="h-[500px] md:h-full w-full relative">
        <div 
          ref={mapRef} 
          className="w-full h-full rounded-r-xl bg-slate-100"
  ...
  <style jsx global>{`
    ...
    input[type="range"]::-webkit-slider-thumb {
      ...
      background: linear-gradient(135deg, #0086C0, #0E374A);
      cursor: pointer;
      box-shadow: 0 2px 6px rgba(0, 134, 192, 0.4);
